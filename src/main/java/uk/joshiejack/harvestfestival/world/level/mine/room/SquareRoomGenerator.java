...
public class SquareRoomGenerator extends RoomGenerator {
    public static final Codec<SquareRoomGenerator> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            Codec.INT.optionalFieldOf("max", 12).forGetter(generator -> generator.max)
    ).apply(instance, SquareRoomGenerator::new));

    private final int max;

    private SquareRoomGenerator(int max) {
        this.max = max;
    }

    public static SquareRoomGenerator squareRoomGenerator(int max) {
        return new SquareRoomGenerator(max);
    }
    public static SquareRoomGenerator squareRoomGenerator() {
        return new SquareRoomGenerator(12);
    }

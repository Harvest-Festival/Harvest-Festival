 ...
 function watch (player, television) {
   var week = 1//Math.floor((calendar.day(player.world()) % 21) / 7)
   if (week < 0) { week = 0 }
   if (week > 2) { week = 2 }
   ...
   print("watching " + item + " on the cooking channel")

   status.set(item, 1) // TODO: Remove the status of the other items
   television.watch('harvestfestival:' + item)
   television.chatterTranslated(player, 'television.harvestfestival.channel.cooking.' + item + '.watch')
 }
